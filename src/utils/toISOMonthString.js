...
import { getMonthFormat } from './calendarSystem';

export default function toISOMonthString(date, currentFormat, calendarSystem) {
  const dateObj = moment.isMoment(date) ? date : toMomentObject(date, currentFormat);
  if (!dateObj) return null;

  return dateObj.format(getMonthFormat(calendarSystem));
